...
module "vpc_subnets_tgw_attachment" {
  ...
  tags                                            = merge(var.tags, tomap({"Name" = var.name_tgw_attachment}))
...
module "vpc_subnets_tgw_attachment_tgw_route_table_association" {
  ...
  tags                           = merge(var.tags, tomap({"Name" = var.name_tgw_route_table_association}))
...
module "vpc_route_consolidation" {
  ...
  source                 = "../route"
  tags                   = merge(var.tags, tomap({"Name" = var.name_route_consolidation}))
  transit_gateway_id     = var.tgw_id
}
